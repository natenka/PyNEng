...
Git сообщает, что мы находимся в ветке master (эта ветка создаётся сама и используется по умолчанию), и что ему нечего добавить в коммит. Кроме этого, Git предлагает создать или скопировать файлы и после этого воспользоваться командой git add, чтобы Git начал за ними следить.
...
Или за всеми файлами в текущей директории рекурсивно:
...
Фраза "nothing to commit, working directory clean" обозначает, что на данный момент нет изменений, которые нужно добавить в Git или закоммитить.
